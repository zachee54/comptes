...
/** <i>Commentaire Olivier Haas : il s'agit d'une classe qui gère les fichiers
 * temporaires en supprimant, au démarrage, les fichiers temporaires rémanents
 * d'une précédente JVM.<br>
 * J'ai trouvé le code dans un article sur devx.com. C'est un site de discussion
 * pour développeurs ; il n'y a pas de licence.<br>
 * Au départ, il s'agit de contourner des bugs de certaines versions de la
 * JVM 7. À l'heure actuelle, le projet est prévu pour tourner sur Java 6u20,
 * donc cela ne concerne pas (encore) l'application Ωmega.<br>
 * En revanche, j'ai un problème similaire pour une raison bête : Ωmega met des
 * fichiers temporaires à disposition de LibreOffice. Mais si la JVM est fermée
 * avant LibreOffice, alors les fichiers ne peuvent pas être effacés puisqu'ils
 * sont ouverts par une autre application. D'où l'intérêt d'utiliser cette
 * classe.<br>
 * L'auteur n'en parle pas, mais je pense que cela sert aussi pour les fichiers
 * rémanents après un plantage.</i>
 * <p>
...
public class TempFileManager
{
   ...
   */
  private final static String TEMP_DIR_PREFIX = "Ωmega-";
  ...
  static
  {
    ...
    for (int i = 0; i < tmpFiles.length; i++)
    {
      ...
      // Edit (Olivier Haas) : supprimer le verrou s'il a plus de 24 heures
      if (lockFile.exists()
    		  && System.currentTimeMillis() - lockFile.lastModified()
    		  		> 24*60*60*1000)
      {
    	  lockFile.delete();
      }
